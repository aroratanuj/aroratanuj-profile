      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-6">
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl md:rounded-3xl p-6 md:p-8 shadow-xl mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 lg:gap-6">
              {domainExpertise.map((domain, index) => (
                <div
                  key={index}
                  className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl md:rounded-2xl p-4 md:p-5 hover:bg-white/20 transition-all duration-300 hover:scale-105"
                >
                  <div className="flex items-start gap-3 md:gap-4">
                    <div className="w-10 h-10 md:w-12 md:h-12 bg-white/20 rounded-lg md:rounded-xl flex items-center justify-center flex-shrink-0 text-2xl md:text-3xl">
                      {domain.icon}
                    </div>
                    <div className="flex-1">
                      <h4 className="text-base md:text-lg lg:text-xl font-bold text-white mb-1 md:mb-2">
                        {domain.name}
                      </h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-6">
          {skills.map((skillGroup, index) => (
            <div
              key={index}
              className="bg-white border border-gray-200 rounded-xl md:rounded-2xl p-4 md:p-5 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 hover:border-primary"
            >
              <div className="w-10 h-10 md:w-12 md:h-12 bg-indigo-100 rounded-lg md:rounded-xl flex items-center justify-center mb-3 md:mb-4">
                <svg className="w-5 h-5 md:w-6 md:h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                </svg>
              </div>
              <h3 className="text-xs md:text-sm font-extrabold text-primary uppercase mb-2 md:mb-3 tracking-wider">
                {skillGroup.category}
              </h3>
              <ul className="space-y-1 md:space-y-2">
                {skillGroup.skills.map((skill, i) => (
                  <li
                    key={i}
                    className="flex items-center gap-1.5 md:gap-2 text-gray-700 text-xs md:text-sm"
                  >
                    <span className="w-1 h-1 md:w-1.5 md:h-1.5 bg-indigo-400 rounded-full flex-shrink-0"></span>
                    {skill}
                  </li>
                ))}
              </ul>
